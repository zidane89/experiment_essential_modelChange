...
class Environment:
    def __init__(self, cell_model, cycle_path, battery_path, motor_path, reward_factor):
        ...
        * (self.v_veh > 0)
        ...
        self.battery = {
            ...
            "Power_min": battery_comp["Pow_min"][0][0][0][0],
        }
    ...
    def get_curdensity_region(self, p_mot):
        ...
        condition_set1 = [True if p_stack >= 0 else False for p_stack in P_stack_set]

        P_battery_set = p_mot - P_stack_set
        condition_set2 = [self.condition_check_battery(p_bat) for p_bat in P_battery_set]

        condition_set = np.array(condition_set1, dtype=np.int) + np.array(condition_set2, dtype=np.int)
        possible_region = np.where(condition_set == 2)[0]
        if len(possible_region) == 0:
            j_fc_min, j_fc_max = self.stack_comp["idling_current_density"], self.stack_comp["max_current_density"]
        else:
            j_fc_set_avail = j_fc_set[possible_region]
            j_fc_min, j_fc_max = j_fc_set_avail[0], j_fc_set_avail[-1]
        return j_fc_min, j_fc_max
        # if np.sum(condition_set) == 0:
        #     # done = True
        #     j_fc_min, j_fc_max = self.stack_comp["idling_current_density"], self.stack_comp["max_current_density"]
